  ...
  const loadEvent = async () => {
    try {
      ...
      if (eventDoc.exists()) {
        ...
        // Verificar si ya existe un comentario del usuario
        const userComment = eventData.comments?.find(
          (c) => c.userId === auth.currentUser?.uid
        );
  ...
  const handleAttendance = async () => {
    try {
      ...
      if (response.ok) {
        ...
        CustomAlert.alert(
          "Error",
          data.error || "No se pudo actualizar tu asistencia"
        );
      }
    } catch (error) {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.organizerRow}>
          ...
          <Text style={styles.organizerText}>
            Organizado por: {event.organizerEmail}
          </Text>
        </View>
                    ...
                    <View style={styles.commentInfo}>
                      ...
                      <View style={styles.commentStars}>
                        {[1, 2, 3, 4, 5].map((star) => (
                          <MaterialIcons
                            key={star}
                            name={c.rating >= star ? "star" : "star-border"}
                            size={14}
                            color="#FFC107"
                          />
                        ))}
          ...
          {showCommentBox && (
            <View style={styles.commentBox}>
              ...
              >
                ...
                <Text style={styles.saveCommentButtonText}>Guardar</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
      ...
      {isOrganizer ? (
        ...
        !isPastEvent && (
          ...
          >
            <MaterialIcons
              name={attending ? "check-circle" : "add-circle"}
              size={18}
              color="#fff"
            />
      ...
      <View style={styles.licenseSection}>
        ...
        <Text style={styles.licenseText}>
          Este contenido está bajo licencia Creative Commons
